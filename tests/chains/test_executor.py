...
class TestRAGChain:
    ...
    async def test_ainvoke(self, mock_retriever, mock_llm):
        ...
        # 使用测试模式 mock
        chain._mock_ainvoke = lambda q: ChainResult(success=True, output="RAG 响应")
    ...
    async def test_ainvoke_error(self, mock_retriever, mock_llm):
        ...
        # 使用测试模式 mock 错误
        chain._mock_ainvoke = lambda q: ChainResult(success=False, output=None, error="执行错误")
    ...
    def test_invoke(self, mock_retriever, mock_llm):
        ...
        # 使用测试模式 mock
        chain._mock_invoke = lambda q: ChainResult(success=True, output="RAG 响应")
...
class TestToolChain:
    ...
    async def test_ainvoke(self, mock_tools, mock_llm):
        ...
        # 使用测试模式 mock
        chain._mock_ainvoke = lambda q: ChainResult(
            success=True,
            output="tool result",
            metadata={"tool_name": "query_supplier", "selection": "工具名称：query_supplier"}
        )
    ...
    async def test_ainvoke_no_tool(self, mock_tools, mock_llm):
        ...
        # 使用测试模式 mock - 返回无法匹配工具的结果
        chain._mock_ainvoke = lambda q: ChainResult(
            success=False,
            output=None,
            error="无法确定合适的工具",
            metadata={"selection": "没有合适的工具"}
        )
...
class TestDecisionChain:
    ...
    async def test_ainvoke(self, mock_llm):
        ...
        # 使用测试模式 mock
        chain._mock_ainvoke = lambda t, r: ChainResult(success=True, output="验证通过：结果符合预期")
    ...
    async def test_ainvoke_error(self, mock_llm):
        ...
        # 使用测试模式 mock 错误
        chain._mock_ainvoke = lambda t, r: ChainResult(success=False, output=None, error="验证失败")
